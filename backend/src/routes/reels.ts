...
import { autoGenerateDesignForReel } from "../lib/canvaDesign.js";
...
reelsRouter.patch("/:id", requireAuth, async (req, res) => {
  ...
  const { data: before } = await req.db!
    .from("reels")
    .select("caption_short, caption_long, canva_design_id")
    .eq("id", req.params.id)
    .single();
  ...
  // Disparador automático (Refinamiento #2): en cuanto los captions quedan
  // completos por primera vez, generamos el diseño en Canva en background
  // — no bloquea la respuesta del auto-guardado de Captions.
  const wasComplete = Boolean(before?.caption_short && before?.caption_long);
  const isNowComplete = Boolean(data.caption_short && data.caption_long);
  if (!wasComplete && isNowComplete && !before?.canva_design_id && features.canva) {
    isCanvaConnected(req.userId!).then((connected) => {
      if (connected) void autoGenerateDesignForReel(req.userId!, data.id);
    });
  }
